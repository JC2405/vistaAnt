...
open() {
    ...
    if (modal) modal.appendChild(this._dropdown);
    else document.body.appendChild(this._dropdown);

    // ✅ Bug 2: esperar al siguiente frame para leer dimensiones reales
   ...
   _reposition() {
    ...
    const dropH = Math.min(maxH, goUp ? above : below);

    this._dropdown.querySelector('.sd-list').style.maxHeight = (dropH - 52) + 'px';

    const w = Math.max(rect.width, 220); // mínimo 220px de ancho
    
    // ✅ Calcular left considerando scroll de la página
    ...
    Object.assign(this._dropdown.style, {
        position : 'fixed',
        width    : w + 'px',
        left     : left + 'px',  // fixed ignora scrollX, corrección:
        top      : goUp
                 ? (rect.top   - dropH - 4) + 'px'
                 : (rect.bottom + 4)   + 'px',
    });

    // Corrección final: con position fixed, left debe ser relativo al viewport
    this._dropdown.style.left = Math.max(8, Math.min(rect.left, window.innerWidth - w - 8)) + 'px';
        }
    close() {
        if (!this._isOpen()) return;
