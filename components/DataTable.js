 ...
 */
export function initTablePagination(id, allData, columns, wrapperSelector) {
    // Guardamos estado en el elemento wrapper para reutilizar
    const wrapper = document.querySelector(wrapperSelector || `#table-container`);
    if (!wrapper) return;
    ...
    function renderPage(page) {
        ...
        // Re-renderizar paginador
        const paginatorEl = wrapper.querySelector('.dt-custom-paginator');
        if (paginatorEl) {
    ...
    function bindClicks() {
        wrapper.querySelectorAll(`.dt-page-btn[data-table="${id}"]`).forEach(btn => {
