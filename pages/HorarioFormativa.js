...
class HorarioFormativa {
    ...
    renderLayout() {
        ...
        document.getElementById('app').innerHTML =
            ...
            '<div class="mb-3" id="container-ambiente"><label class="form-label small text-muted">Ambiente (Sede Activa)</label>' +
            '<select class="form-select" id="idAmbiente" required><option value="">Seleccionar ambiente...</option></select></div>' +

            '<div class="mb-4"><label class="form-label small text-muted">Instructor</label>' +
            '<select class="form-select" id="idFuncionario" required><option value="">Seleccionar instructor...</option></select></div>' +
            ...
            '<div class="mb-4"><label class="form-label small text-muted d-block mb-2">4. Días de la semana</label>' +
            '<div class="d-flex flex-wrap gap-2" id="dias-container"></div></div>' +
            ...
            '<i class="bi bi-calendar-check"></i> Asignar</button>' +
            '</div></div>' +
    ...
    renderInstructores(idAreaPreferida = null) {
        ...
        if (idAreaPreferida) {
            sorted.sort((a, b) => {
                const aHas = a.areas && a.areas.some(ar => String(ar.idArea) === String(idAreaPreferida));
                const bHas = b.areas && b.areas.some(ar => String(ar.idArea) === String(idAreaPreferida));
    ...
    setupEventListeners() {
        ...
        document.getElementById('idAmbiente')?.addEventListener('change', e => {
            const ambObj = this.ambientes.find(a => String(a.idAmbiente) === String(e.target.value));
            this.renderInstructores(ambObj?.idArea ?? null);
        });
    ...
    renderBreadcrumb() {
        ...
        if (['programas', 'fichas', 'horario'].includes(this.viewState) && this.selectedSede) {
            if (this.viewState === 'programas') {
                html += '<li class="breadcrumb-item active">' + this.selectedSede.nombre + '</li>';
            } else {
        ...
        if (['fichas', 'horario'].includes(this.viewState) && this.selectedPrograma) {
            if (this.viewState === 'fichas') {
                html += '<li class="breadcrumb-item active">' + this.selectedPrograma.nombre + '</li>';
            } else {
        ...
        if (this.viewState === 'horario' && this.selectedFicha) {
            html += '<li class="breadcrumb-item active">Ficha ' + this.selectedFicha.codigoFicha + '</li>';
        }
        bc.innerHTML = html;

        document.getElementById('bc-sedes')?.addEventListener('click', e => { e.preventDefault(); this.setViewState('sedes'); });
        document.getElementById('bc-programas')?.addEventListener('click', e => { e.preventDefault(); this.setViewState('programas'); });
    ...
    renderSedesView(container) {
        if (!this.sedes.length) { container.innerHTML = '<div class="alert alert-info">No hay sedes disponibles.</div>'; return; }
        ...
        container.querySelectorAll('.card').forEach(card => {
            card.addEventListener('click', () => {
                this.selectedSede = this.sedes.find(s => String(s.idSede) === card.dataset.id);
                this.renderAmbientes();
    ...
    renderProgramasView(container) {
        ...
        const propsById = {};
        myFichas.forEach(f => {
            if (!propsById[f.idPrograma]) propsById[f.idPrograma] = { programa: f.programa, fichasCount: 0 };
        ...
        const renderProgCards = (programs) => programs.map(({ programa, fichasCount }) => {
            const tipo = programa.tipoFormacion ? programa.tipoFormacion.nombre : 'Formativa';
            let colorClass = 'primary';
            if (tipo.toLowerCase().includes('tecnólogo')) colorClass = 'success';
            else if (tipo.toLowerCase().includes('técnico')) colorClass = 'info';
            else if (tipo.toLowerCase().includes('operario')) colorClass = 'warning';
            return `<div class="col-md-6 col-lg-4 prog-card">
        ...
        container.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-4">
                <button class="btn btn-light rounded-pill border shadow-sm" id="btn-back-sedes"><i class="bi bi-arrow-left me-2"></i>Volver</button>
                <div class="input-group" style="max-width:320px;">
                    <span class="input-group-text bg-white"><i class="bi bi-search text-muted"></i></span>
                    <input type="text" class="form-control" id="search-programa" placeholder="Buscar programa...">
                </div>
            </div>
            <div class="row g-4" id="programas-grid">${renderProgCards(distinctPrograms)}</div>`;
        ...
        const attachCardEvents = () => {
            container.querySelectorAll('.card.hover-lift').forEach(card => {
                card.addEventListener('click', () => {
                    const p = distinctPrograms.find(dp => String(dp.programa.idPrograma) === card.dataset.id);
                    if (p) { this.selectedPrograma = p.programa; this.setViewState('fichas'); }
                });
                card.addEventListener('mouseenter', () => card.style.transform = 'translateY(-5px)');
                card.addEventListener('mouseleave', () => card.style.transform = 'translateY(0)');
            });
        ...
        let html = `
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div class="d-flex align-items-center gap-3">
                    <button class="btn btn-light rounded-pill border shadow-sm" id="btn-back-prog"><i class="bi bi-arrow-left me-2"></i>Volver</button>
                    <div>
    ...
    async selectFicha(idFicha) {
        ...
        card.innerHTML = '<div class="card-body p-5 text-center d-flex flex-column align-items-center justify-content-center">' +
            '<div class="spinner-border text-primary mb-3" role="status"></div>' +
            '<p class="text-muted small">Cargando horario de ' + this.selectedFicha.codigoFicha + '...</p></div>';
    ...
    renderGrid(ficha, grilla) {
        ...
        if (isEmpty) {
            body += '<div class="text-center py-5 text-muted h-100 d-flex flex-column justify-content-center">' +
                ...
                '<p class="small">Usa "Agregar Clase" para comenzar.</p></div></div>';
        ...
        // Agrupar eventos: clave = idBloque_dia (para que cada día sea un evento independiente)
        const groupedEvents = {};
        ...
        const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                return {
                    html: `
                        <div class="p-1 h-100 d-flex flex-column position-relative" style="overflow:hidden;">
                            <div class="fw-bold mb-1 lh-sm" style="font-size:0.8rem;">${props.instructor}</div>
            ...
            eventMouseEnter: function (info) {
                info.el.querySelector('.delete-dia-btn')?.classList.remove('d-none');
            },
            eventMouseLeave: function (info) {
                info.el.querySelector('.delete-dia-btn')?.classList.add('d-none');
            }
        });

        calendar.render();

        // ── Delegar el click en el botón ❌ → eliminar solo ese día del bloque
